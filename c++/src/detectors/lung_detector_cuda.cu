		                               ...
		                               unsigned int back_threshold)
{
	...
	unsigned int v_idx = blockIdx.x * blockDim.x * blockDim.x +
                         threadIdx.x;
...
{
	if(slice_bounds[0] <= c_idx and c_idx <= slice_bounds[1] and
	   slice_bounds[2] <= r_idx and r_idx <= slice_bounds[3])
		return true;
	return false;
}
		                     ...
		                     const float *lung_assumed_c_n)
{
	...
	else
	{
		...
		for(unsigned int i = 0; i < count; i++)
		{
			...
			central_r[i] = 0.0;
			central_s[i] = 0.0;
			center_of_mass(labeled, size, main_labels[i],
						   central_c[i], central_r[i], central_s[i]);
		}
		//___________________________________________________________________//
		...
		if(central_c[0] < (1. - center_c_th) and
		   central_c[0] > center_c_th and
		   central_r[0] < (1. - center_r_th) and
		   central_r[0] > center_r_th and
		   central_s[0] > slice_th and
		   (main_candidates[0] / main_candidates[1]) > size_f)
		{
			extract_lung_labels(labeled, candidates, size, main_labels, 1);
			goto end_detection2;
		}
		...
		else
		{
			...
			for(unsigned int i = 0; i < (count - 1); i++)
			{
				...
				{
					if(((central_c[i] < center_c_th and
					     central_c[j] > (1. - center_c_th)) or
						(central_c[j] < center_c_th and
						 central_c[i] > (1. - center_c_th))) and
					   abs(central_r[i] - central_r[j]) < center_r_d and
					   abs(central_s[i] - central_s[j]) < center_s_d)
					{
						main_labels[0] = main_labels[i];
						main_labels[1] = main_labels[j];
						extract_lung_labels(labeled, candidates,
								            size, main_labels, 2);
						goto end_detection2;
					}
				}
			...
			{
				current_dist =
						sqrt(pow(central_c[i] - lung_assumed_c_n[0], 2) +
			       ...
			       float air_threshold)
{
	...
	unsigned int s_volume_s[3] = {volume_s[0] / subsample_f[0],
			                      volume_s[1] / subsample_f[1],
	...
	labeling_3d(const_cast<const bool *>(air_mask_sub),
			    labeled, s_volume_s, object_sizes, label);
	delete [] air_mask_sub;
	//_______________________________________________________________________//
	...
	float s_lung_volume_threshold = lung_volume_threshold / (subsample_f[0] *
			                                                 subsample_f[1] *
			                                                 subsample_f[2]);
	...
	labeling_3d(const_cast<const bool *>(air_mask),
			    labeled, volume_s, object_sizes, label);

	candidates = new bool[volume_l];
	extract_lung_candidates(const_cast<const unsigned int *>(labeled),
			                volume_s, object_sizes, label,
			                candidates, lung_volume_threshold,
			                lung_assumed_center_n);
