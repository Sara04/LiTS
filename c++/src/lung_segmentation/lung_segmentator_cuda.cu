#include "lung_segmentator_cuda.cuh"
...
/*
 * find lung central slice and marge
 */
void lung_central_slice(bool *air_mask, unsigned int * volume_s,
                        float &lung_center_slice)
{
    // 1. Average area over slice
    float *coronal_sum = new float[volume_s[2]];
    for(unsigned int s = 0; s < volume_s[2]; s++)
    {
        coronal_sum[s] = 0;
        for(unsigned int i = 0; i < (volume_s[0] * volume_s[1]); i++)
            coronal_sum[s] += air_mask[s * volume_s[0] * volume_s[1] + i];

        coronal_sum[s] /= (volume_s[0] * volume_s[1]);
    }
    ...
    // 3. Count candidate regions
    unsigned int count_sao = 0;
    bool sao_start = false;
    bool sao_end = false;
    for(unsigned int s = 0; s < volume_s[2]; s++)
    {
        if(coronal_sum[s] <= th)
        ...
        if(sao_start and (sao_end or s == (volume_s[2] - 1)))
        {
            count_sao += 1;
            sao_start = false;
            sao_end = false;
    ...
    // 4. Compute "mass" and vertical "mass_centers" of candidates
    float *sao_mass_centers = new float[count_sao];
    unsigned int *sao_mass_start = new unsigned int[count_sao];
    unsigned int *sao_mass_end = new unsigned int[count_sao];

    float *sao_mass = new float[count_sao];
    unsigned int sao_s = 0;
    count_sao = 0;
    for(unsigned int s = 0; s < volume_s[2]; s++)
    {
        if(!coronal_sum[s])
        ...
        else
        {
            ...
            {
                sao_start = true;
                sao_mass_centers[count_sao] = 0;
                sao_mass[count_sao] = 0;
                sao_mass_start[count_sao] = s;
            }
            sao_mass_centers[count_sao] += (float(s) * coronal_sum[s]);
            sao_mass[count_sao] += coronal_sum[s];
            sao_s += 1;
        }

        if(sao_start and (sao_end or s == (volume_s[2] - 1)))
        {
            sao_mass_end[count_sao] = s;
            sao_mass_centers[count_sao] /= (sao_mass[count_sao]);
            sao_mass[count_sao] /= (sao_mass_end[count_sao] - sao_mass_start[count_sao]);
            count_sao += 1;
            sao_s = 0;
            sao_start = false;
            sao_end = false;
    ...
    // 5. Find the largest air region
    unsigned int l_idx = 0;
    float l = sao_mass[l_idx];
    for(unsigned int i = 1; i < count_sao; i++)
    {
        if(sao_mass[i] >l)
        {
            l_idx = i;
    ...
    lung_center_slice = sao_mass_centers[l_idx];
    // 6. Find the bottom lungs bound
    unsigned int lungs_bottom;
    lungs_bottom = (unsigned int)(float(sao_mass_start[l_idx]) -
                                  0.3 * float(sao_mass_centers[l_idx] -
                                              sao_mass_start[l_idx]));


    // 7. Remove all air objects below bottom lungs bound
    for(unsigned int s = 0; s < volume_s[2]; s++)
    {
        for(unsigned int i = 0; i < (volume_s[0] * volume_s[1]); i++)
        {
            if(s < lungs_bottom)
                air_mask[s * volume_s[0] * volume_s[1] + i] = 0.0;
        }
    }
                   ...
                   float lung_volume_threshold, float air_threshold)
{
    ...
    lung_central_slice(air_mask_sub,  s_volume_s, lung_center_slice);
    lung_assumed_center_n[2] = lung_center_slice / s_volume_s[2];
