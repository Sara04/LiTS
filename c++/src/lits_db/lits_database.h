 ...
 * subsets;
 * getting the volume and segmentation paths of a training scan;
 * getting the volume path of a testing volume
 *
 *
 * Attributes:
 ...
 * Methods:
 * 		LiTS_db: constructor
 * 		load_train_subject_names: loading train subjects' names
 * 		load_test_subject_names: loading test subjects' names
 ...
 *
 * 		get_train_subject_name: get the subject's name from the training set at
 * 			required position
 * 		get_develop_subject_name: get the subject's name from the development
 * 			set at required position
 * 		get_valid_subject_name: get the subject's name from the validation set
 * 			at required position
 * 		get_eval_subject_name: get the subject's name from the validation set
 * 			at required position
 * 		get_test_subject_name: get the subject's name from the testing set
 * 			at required position
 *
 * 		get_train_paths: get subject's volume and segmentation paths
 * 		get_test_path: get subject's volume path
 */
class LiTS_db
{
private:
